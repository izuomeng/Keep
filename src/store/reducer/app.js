...
export default (state = {}, action) => {
  switch (action.type) {
    ...
    case Types.ADD_DOCUMENT_CLICK_HANDLERS:
      const newHandlers = state.documentClickHandlers.concat(action.handler)
      return {...state, documentClickHandlers: newHandlers}
    case Types.REMOVE_DOCUMENT_CLICK_HANDLERS:
      return {
        ...state,
        documentClickHandlers: state.documentClickHandlers.filter(v => {
          return v !== action.handler
        })
      }
    default:
        return state
