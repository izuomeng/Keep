...
const Day = Week.extend`
  font-weight: normal;
  cursor: ${props => 
    props.prev || props.highLight ? 'default' : 'pointer'};
  color: ${props => 
    props.prev ? 'grey' : (props.highLight ? 'white' : 'black')};
  background: ${props => 
    props.highLight ? 'linear-gradient(to bottom, #FEBA2C, #D99316)' : ''};
  &:hover {
    background: ${props => 
      props.prev || props.highLight ? '' : '#eeeeee'};
  }
`
class DatePicker extends Component {
  constructor(props) {
    ...
    if (isLeapYear(this.fullYear)) this.HowManyDays[2] = 29
  ...
  calcRenderData(year, month) {
    if (isLeapYear(year)) this.HowManyDays[2] = 29
    else this.HowManyDays[2] = 28
  ...
  onDateClick(date) {
    return () => {
      ...
      this.setState({date})
      this.props.onSelectDate(this.pickedDate)
    }
  }
  onNextClick() {
    ...
    return (
      <Container show={show}>
        <Top data-id='setReminder'>
          ...
          <Main style={{padding: 0, clear: 'both'}}>
            {Weeks.map(v => (
              <Week data-id='setReminder'>{v}</Week>
            ))}
          </Main>
        </Top>
        <Main data-id='setReminder'>
          {renderData.map((v, i, ary) => {
            if (v === this.pickedDate.date && i >= ary.start && 
              i < ary.stop && this.isPicked()) {
              return (
                <Day data-id='setReminder' highLight key={Math.random()}>
                  {v}
                </Day>
              )
            }
            if (i < ary.start || i >= ary.stop) {
              return <Day data-id='setReminder' prev key={Math.random()}>{v}</Day>
            }
            return (
              <Day
                key={Math.random()}
