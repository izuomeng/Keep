...
class WaterFall extends Component {
  ...
  constructor(props) {
    super(props)
    this.waterDropStyle = this
      .props
      .notes
      .map((v) => ({width: 240, height: v.height, key: v.id}))
    const windowWidth = this.getWidth()
    if (window.innerWidth > minWidthToHideSidebar) {
      this.containerWidth = this.props.sidebar
        ? windowWidth - 340
        : windowWidth - 60
    } else {
      this.containerWidth = windowWidth - 80
    }
    const layout = this.computeXY(
      this.waterDropStyle, 
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const waterDropStyle = nextProps
      .notes
      .map((v) => ({width: 240, height: v.height, key: v.id})),
      width = this.containerWidth,
      layout = this.computeXY(waterDropStyle, {width})
  ...
  getSelectInfo(info, id) {
    if (info) {
      return info.find(v => v.id === id)
    }
  }
  render() {
    const {notes, layout} = this.state
    const {label, selectInfo} = this.props
    return (
      <WaterFallContainer>
        {label && <Label left={layout[0].left}>
          {label}
        </Label>}
        <Container height={this.height}>
          {layout.map((v, i) => (
            <Wrapper top={v.top} left={v.left} key={v.key}>
              <Card
                note={notes[i]}
                selectInfo={this.getSelectInfo(selectInfo, notes[i].id)}/>
            </Wrapper>
          ))}
        </Container>
      </WaterFallContainer>
...
const mapState = (state) => ({sidebar: state.app.sidebar})
